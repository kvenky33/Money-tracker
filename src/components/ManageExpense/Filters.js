  ...
  const handleFilters = () => {
    const newData = store.filter((expenseObj) => {
      if (
        ...
        expenseObj.date.substr(0, 4) === year &&
        expenseObj.date.substr(5, 2) === month
      )
    ...
    <Card size="md" mr={12} ml={12} mb={30}>
      <CardBody>
        <Flex justifyContent="space-around">
          ...
          <Flex gap={10}>
            <Button mt={4} colorScheme="twitter" onClick={handleReset}>
              Reset
            </Button>
            <Button mt={4} colorScheme="twitter" onClick={handleFilters}>
              Search
            </Button>
          </Flex>
        </Flex>
